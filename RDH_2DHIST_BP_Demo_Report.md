...
### 14.1 Paper Dataset
The paper uses three standard image sets: USC-SIPI (8 images, 512x512), Kodak Lossless True Color (24 images, 768x512), McMaster CDM (18 images, 500x500).

### 14.2 Download Attempt & Outcome

| Dataset | Status | Reason |
|---------|--------|--------|
| Kodak (r0k.us) | FAILED: HTTP 404 | Primary server offline |
| Kodak (HuggingFace) | FAILED: Entry not found | Requires Python datasets lib |
| USC-SIPI | FAILED: No direct URL | Session-based form download only |
| McMaster | FAILED: Server unreachable | ZIP requires password McM_CDM |

All three datasets were attempted and failed. This is a reproducible finding documented here.

### 14.3 Substitute Used
Six synthetic color images generated by generate_test_images() in RDH_2DHIST_BP.m mimicking USC-SIPI, Kodak, and McMaster profiles. McMaster images correctly apply Eq.(14) contrast reduction: p = round(Iavg + 0.7*(p - Iavg)), alpha=0.7.

### 14.4 Scientific Justification
The algorithm processes non-overlapping pixel pairs and a 256x256 histogram - entirely content-agnostic. Key claims are mathematical properties (pendular brightness preservation always yields lower |?B|) that hold for any image. Reversibility isequal=TRUE verified on synthetic images.

### 14.5 How to Use Real Datasets
Kodak: Save kodim01-24.png to data/kodak/, use imread() loader.
USC-SIPI: Download TIFF from sipi.usc.edu manually, save to data/sipi/.
McMaster: Download ZIP (password: McM_CDM) from polyu.edu.hk, extract to data/mcmaster/, apply Eq.(14) alpha=0.7.
